...
let dictionary = fs
    .readFileSync('../dictionary/dictionary.txt', { encoding: 'UTF8' })
    .replace('\r', '')
    .split('\n');

let ocDictionary = fs
    .readFileSync('../dictionary/opencorekeys.txt', { encoding: 'UTF8' })
    .replace('\r', '')
    .split('\n');
...
dictionary = dictionary.filter(
    (string, index) => dictionary.indexOf(string) == index,
);
ocDictionary = ocDictionary.filter(
    (string, index) => ocDictionary.indexOf(string) == index,
);
